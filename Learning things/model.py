...
def predict_single_input(single_input):
    # Create dataframe of the input
    new_input_df = pd.DataFrame([single_input])

    # Apply data preparation to an input
    new_input_df[numeric_cols] = imputer.transform(new_input_df[numeric_cols])
    new_input_df[numeric_cols] = scaler.transform(new_input_df[numeric_cols])
    new_input_encoded = pd.DataFrame(encoder.transform(new_input_df[categorical_cols]),
                                columns=encoded_cols,
                                index=new_input_df.index
    )
    new_input_df = pd.concat([new_input_df[numeric_cols], new_input_encoded], axis=1)
    X_input = new_input_df[numeric_cols + encoded_cols]

    # Run prediction and confidence evaluation
    pred = model.predict(X_input)
    prob = model.predict_proba(X_input)[0][list(model.classes_).index(pred)]
    print(pred)
    print(prob)

# # Creating single input
...
# predict_singel_input(single_input)
...
predict_single_input(new_input)
